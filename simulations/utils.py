...
def get_random_covs(p, rank, nenvs, rng, a1=0.1, b1=1.0, a2=0.1, b2=1.0,
                    env_eigs_vary=False):
    """
    ...
    # Shared eigenvalues + basis
    eigvals_shared = rng.uniform(a1, b1, rank)
    Q0, _ = np.linalg.qr(rng.standard_normal((p, rank)))

    # Generate env-specific eigenvalues (once if not varying)
    if not env_eigs_vary:
        eigvals_env = rng.uniform(a2, b2, rank)

    for _ in range(nenvs):
        # If env_eigs_vary, generate new env-specific eigenvalues
        if env_eigs_vary:
            eigvals_env = rng.uniform(a2, b2, rank)

        # Environment-specific orthonormal basis, orthogonal to Q0
        Qi = rng.standard_normal((p, rank))
        # Make orthogonal to Q0
        Qi -= Q0 @ (Q0.T @ Qi)
        # Orthonormalize
        Qi, _ = np.linalg.qr(Qi)

        # Stack into single orthonormal basis
...
def sample_from_convex_hull(item_list, n_samples, rng):
    """
    ...
    while n_samples_drawn < n_samples:
        weights = rng.gamma(shape=1.0, scale=1.0, size=n_items)  # Exp(1)
        weights /= weights.sum()
        cov = np.sum([w * item for w, item in zip(weights, item_list)], axis=0)
        
        if np.any(np.linalg.eigvals(cov) < -1e-8):
            print(f"Sampled covariance is not positive definite, resampling...")
            continue
