...
def MW_for_PCA(n,m,d,B, weight=None, alpha=None, beta=None, eta=1, T=10, verbose = False):
    ...
    for t in range(T):
        ...
        if (verbose):
            ...
            print(this_t_stats)
        run_stats.append(pd.DataFrame(this_t_stats,index=[t]))
...
def fairDimReduction_MW(n,k,d,B,Obj='MM_Loss',eta=1, T=10, verbose = False, timed = True):
    ...
    Arguments:
        ...
        Obj:the objective to optimize. Must be MM_Var (maximize the minimum variance)
          or MM_Lose (default) (minimize the maximum loss, output the negative number variance - best)
        eta: learning rate of MW.
        T: number of iteration
    ...
    if (timed):
        ...
        print("fairDimReduction_MW is called. Total time used is: ", 
              total_time, " seconds.")
        return [X_last, X_avg, Obj, total_time]
